...
ObjectView::ObjectView(QObject* parent, const QString & fileName) : QObject(parent) {
  ...
  if (!file.open(QFile::ReadOnly | QFile::Text)) {
    QMessageBox::warning(qApp->activeWindow(), tr("XSL Stylesheet"), tr("Cannot read file %1:\n%2.")
                                                        .arg(fileName)
  ...
  if (!objectFile.setContent(&file, true, &errorStr, &errorLine, &errorColumn)) {
    QMessageBox::information(qApp->activeWindow(), tr("XSL Stylesheet"), tr("Parse error at line %1, column %2:\n%3")
																.arg(errorLine)
                              									.arg(errorColumn)
																.arg(errorStr));
    return;
