...
void QDocumentSearch::clearMatches()
{
	...
	QFormatFactory *f = m_editor->document()->formatFactory();
...
void QDocumentSearch::next(bool backward, bool all)
{
	...
	*/
	...
	while ( !end(backward) )
	{
		...
		if ( backward )
		{
			s = l.text().left(m_cursor.columnNumber());
		...
		*/
		...
		if ( column != -1 )
		{
			...
			if ( backward )
			{
				if ( bounded && (boundaries.startLine == ln) && boundaries.start )
					column += boundaries.start;
			...
			if ( hasOption(Replace) )
			{
				...
				// TODO : add temporary overlays on all matches...
				
				QFormatRange r(m_cursor.anchorColumn(), m_cursor.columnNumber() - m_cursor.anchorColumn(), m_editor->document()->formatFactory()->id("search"));
				//qDebug("(%i, %i, %i)", r.offset, r.length, r.format);
	...
	if ( !hasOption(Replace) && hasOption(HighlightAll) )
	{
		m_index = backward ? m_highlight.count() : -1;
		return next(backward);
	}
	
	if ( !found && !hasOption(Silent) )
	{
