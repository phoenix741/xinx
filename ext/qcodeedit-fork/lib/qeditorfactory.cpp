	...
	QSettingsClient(s, "editor")
{
	...
	m_formatFactory = new QFormatFactory(QCE::fetchDataFile("formats.qxf"), this);
	...
	*/
	...
	QDocument::setFormatFactory(m_formatFactory);
	
	m_languageFactory = new QLanguageFactory(m_formatFactory, this);
	...
	/*
	m_languageFactory->addDefinitionPath(
						QApplication::applicationDirPath()
						+ QDir::separator()
						+ "qxs");
...
void QEditorFactory::saved(QEditor *e, const QString& f)
{
	...
	QHighlighterInterface *iface = e->highlighter() ? e->highlighter()->iface() : 0;
	
	m_languageFactory->setLanguage(e, f);
	
	if ( iface != (e->highlighter() ? e->highlighter()->iface() : 0) )
		e->highlight();
