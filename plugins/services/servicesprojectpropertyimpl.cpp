...
ServicesProjectPropertyImpl::ServicesProjectPropertyImpl( QWidget* parent, Qt::WFlags fl ) : QWidget( parent, fl ), Ui::ServicesProjectProperty() {
	...
	connect( m_servicesList->m_list, SIGNAL(itemChanged(QListWidgetItem*)), this, SLOT(on_m_refreshWSDL_clicked()) );
	connect( m_servicesList->m_list, SIGNAL(currentRowChanged(int)), this, SLOT(wsdlSelectionChanged()) );
}
