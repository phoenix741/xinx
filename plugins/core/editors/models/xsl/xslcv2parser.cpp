...
XslContentView2Parser::XslContentView2Parser( bool persistent ) : ContentView2::Parser( persistent ), m_codec( 0 ) {
}
	...
	while( !atEnd() ) {
		...
		if( isStartElement() ) {
			if( ( QXmlStreamReader::name() == "param" ) || ( QXmlStreamReader::name() == "variable" ) ) {
				...
				s.src = ( rootNode().isValid() ? rootNode().fileName() : filename() ) + ".js";
				scripts += s;
				...
				s.content = readElementText();
				s.src = ( rootNode().isValid() ? rootNode().fileName() : filename() ) + ".css";
				scripts += s;
			} else
...
void XslContentView2Parser::readTemplate() {
	...
	foreach( const QString & name, templates ) {
		...
		foreach( struct_script s, scripts ) {
			...
			ContentView2::Parser * parser = 0;
			if( ft && ( parser = ft->createParser2() ) ) {
				try {
